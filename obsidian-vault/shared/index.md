---
title: "Shared Knowledge Base"
type: index
department: "shared"
updated: 2026-04-07
---

# Shared Knowledge Base

Cross-department knowledge, policies, and escalation protocols available to all agents.

## Agent Skills

- [[skills/shared/rag-retrieval|RAG Retrieval]] — How agents search documents and chat history
- [[skills/shared/excel-navigation|Excel Navigation]] — How agents find cells in ALCO Tracker
- [[skills/shared/escalation-protocol|Escalation Protocol]] — Breach detection and notification tiers
- [[skills/shared/citation-format|Citation Format]] — Source attribution standards

## Policies

Add policy documents to `shared/policies/`. They are automatically ingested into the knowledge base within 60 seconds.

## Escalation Protocols

Add escalation protocol documents to `shared/escalation-protocols/`.

## Operations Log

- [[shared/log|Operations Log]]
