---
title: "CEO Knowledge Base"
type: index
department: "ceo"
updated: "2026-05-17"
---

# CEO Knowledge Base

CEO / Board-level knowledge base for Brooker Group PCL. Strategy, board resolutions,
governance, and the corporate worldview — compiled from the `O:\brooker_database\ceo`
source files.

## Sections

- **Concepts** — `ceo/concepts/` — Strategic frameworks and doctrines
  - Strategy & operating model: [[three-engine-model]] · [[north-star-2028]] · [[brooker-operating-system]]
  - Worldview & narrative: [[brooker-worldview-2026]] · [[corporation-2-0]] · [[brooker-vessel-framework]]
  - Capital & regulatory: [[stay-liquid-doctrine]] · [[investment-company-40-percent-rule]]
- **Decisions** — `ceo/decisions/` — The Khao Yai Resolutions (21 Feb 2026)
  - [[2026-02-21-three-engine-model-adoption]] *(R-01)* · [[2026-02-21-melt-up-divestment-plan]] *(R-02)* · [[2026-02-21-capital-sovereignty-resolution]] *(R-03)* · [[2026-02-21-prediction-market-ai-trading]] *(R-04)*
  - [[2026-02-21-committee-governance-structure]] *(R-05)* · [[2026-02-21-strategic-partner-map]] *(R-06)* · [[2026-02-21-2026-okrs]] *(R-07)* · [[2026-02-21-equity-story-rerating]] *(R-08)*
  - [[2026-02-21-strategic-stop-list]]
- **Meeting Notes** — `ceo/meeting-notes/`
  - [[2026-02-21-khao-yai-strategic-retreat]] *(Board strategic retreat, 12 sessions)*
- **Entities** — `ceo/entities/`
  - Corporate: [[brooker-group]] · [[bicl]] · [[brook-technology-capital-vcc]]
  - Funds: [[yield-fund-of-funds]]
  - Placement agents: [[eastbound]] · [[finex]]
- **Trends** — `ceo/trends/`
  - [[2026-budget]] · [[proforma-balance-sheet-2026]]

## Source Documents

Compiled from 5 source files in `O:\brooker_database\ceo` — registered in
`config/document_inventory.json`:

- `BOD Khao Yai Retreat Pack 2026.docx` — the keystone strategic retreat pack (12 sessions, 8 resolutions)
- `Brook Strategic Retreat Khao Yai 2026.pptx` — companion retreat deck
- `The Brooker Worldview 2026.docx` — the 10 worldview theses
- `AI Agent Digital Twin Orchestration Platform.docx` — internal operating model
- `DA budget plan 2026.xlsx` — Digital Asset division budget
